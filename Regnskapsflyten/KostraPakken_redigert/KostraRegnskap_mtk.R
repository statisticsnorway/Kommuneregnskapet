...
#' @export
KostraRegnskapBeregning = function(data,funksjonshierarki,artshierarki,data_saer, data_konsolidert,
  ...
  KostraRegnskap(data=data,funksjonshierarki=funksjonshierarki,artshierarki=artshierarki,
                 data_saer=data_saer, data_konsolidert=data_konsolidert, artshierarki_nettinger=artshierarki_nettinger, artshierarki_nettinger_kasse = artshierarki_nettinger_kasse,
...
CharacterReCodeType = function(x){
  x$type = CharacterReCode(x$type,
    c("OUTPUT_Konsern", "OUTPUT_Kasse", "OUTPUT_Konsolidert", "OUTPUT_Saer",  "OUTPUT_Nett_Saer",  "OUTPUT_Nett_Kasse", "Kasse", "Saer", "Konsolidert",  "Nett_Saer",  "Nett_Kasse", "Input_Kasse", "Input_Saer", "Input_konsolidert", "Konsern", "formel"),
    c("OUTPUT_konsern", "OUTPUT_kasse", "OUTPUT_Konsolidert", "OUTPUT_sbedr", "OUTPUT_nett_sbedr", "OUTPUT_nett_kasse", "kasse", "sbedr", "Konsolidert", "nett_sbedr", "nett_kasse", "input_kasse", "input_sbedr", "Input_konsolidert", "konsern", "formel"))
...
KostraRegnskap = function(data,funksjonshierarki,artshierarki,data_saer=NULL,
  ...
  SjekkDataPerioder(data_saer, "data_konsolidert", perioder)  
  ...
  for(i in seq_len(n)){
    ...
    ai =  KostraRegnskapEnPeriode(data = DataPeriode(data,perioder[i]),
                   ...
                   data_konsolidert = DataPeriode(data_konsolidert,perioder[i]),
...
#' @param data_konsolidert inputdata for konsolidert
...
KostraRegnskapEnPeriode = function(data,funksjonshierarki,artshierarki,data_saer=NULL,
  ...
  if(printData){
    ...
    PrintHeadTail(data_konsolidert)
  ...
  if(!is.null(regnskapsomfanger))
    if(smartB)
      if(!(length(regnskapsomfanger)==1 & (regnskapsomfanger[1]=="B" | regnskapsomfanger[1]=="C")) ){
  ...
  if(!is.null(regnskapsomfanger)){
    ...
    if(any(!(regnskapsomfanger %in% c("A","B","C"))))
      stop(paste("Feil regnskapsomfang:",
                 paste(regnskapsomfanger[!(regnskapsomfanger %in% c("A","B","C"))],collapse=", ")))
...
#Kopierer onlyB til onlyC
        onlyC = FALSE
  if(!is.null(regnskapsomfanger))
    if(regnskapsomfanger=="C")
      onlyC = TRUE
  ...
  periode = unique(c(as.character(data$periode),
                     ...
                     as.character(data_konsolidert$periode),
  ...
  if(output=="beregningHierarki"){
    ...
    beregnetHierarki <-  BeregnetFormelHierarki(data,funksjonshierarki,artshierarki,
                                  data_saer,data_konsolidert, artshierarki_nettinger,artshierarki_nettinger_kasse,
                                  periode = periode,
                                  kombinasjoner=kombinasjoner,
                                  regioner=regioner,
                                  form=form,
                                  toKoder = TRUE, doStack = TRUE,
                                  fixHierarchy = FALSE, onlyC=onlyC)
    if(!beregningHierarkiInput)
      return(beregnetHierarki)    # bruk returnMatrixComponents
    ...
    beregnetHierarki_type = CharacterReCode(beregnetHierarki$type_from, c("Input_Kasse", "Input_Saer", "Input_konsolidert"), c("Kasse", "Saer", "Konsolidert"))
    ...
    krMC = KostraRegnskap(data, funksjonshierarki, artshierarki, data_saer, data_konsolidert,
    ...
    if("formel" %in% colnames(fromMatrix)){
      for(i in which(fromMatrix[, "formel"] != "")){
        krBI = KostraRegnskap(data, funksjonshierarki, artshierarki, data_saer, data_konsolidert,
                              artshierarki_nettinger, artshierarki_nettinger_kasse,
  ...
  if(returnMatrixComponents|beregningInput){
    ...
    return(list(a1=a1,a2=a2,a3=a3,a4=a4))
      ...
      if(onlyC)
        return(LagBeregningInputC(a1=a1,periode=periode))
      return(LagBeregningInput(a1=a1,a2=a2,a3=a3,a4=a4, periode=periode))
    }
  ...
  if(smartB){
    ...
    makeMatDiff = TRUE
    if(!is.null(regnskapsomfanger))
      if(regnskapsomfanger=="B" | regnskapsomfanger=="C")
        makeMatDiff = FALSE
  ...
  if(output=="fire"){
    z=data.frame(kasse = as.vector(as.matrix(mat1)),
                 konsern = as.vector(as.matrix(mat1)),
    ...
    if(smartB & !is.null(regnskapsomfanger)){
      ...
      if(regnskapsomfanger=="C"){
        z=data.frame(a=as.vector(as.vector(as.matrix(mat1))),stringsAsFactors=stringsAsFactors)
        regnskapsomfang =  data.frame(a=rep(c("C"), times = 1, each = cumprod(dim(mat1))[2]  ),stringsAsFactors=stringsAsFactors)
      }

    } else{
      z=data.frame(a=as.vector(c(as.vector(as.matrix(matDiff+mat1)),as.vector(as.matrix(mat1)))),stringsAsFactors=stringsAsFactors)
      regnskapsomfang =  data.frame(a=rep(c("A","B","C"), times = 1, each = cumprod(dim(mat1))[2]  ),stringsAsFactors=stringsAsFactors)
    }
    names(z) = valueVar
